...
class FedALAClient(BaseClient):
    ...
    def adaptive_local_aggregation(self):
        """ Learns per-layer adaptive interpolation weights between global and local parameters.
        ...
        # Prepare optimizer (not used for learning, weights are updated manually)
        optimizer = torch.optim.SGD(params_tp, lr=0)  # lr=0 because we update via weights, not optimizer
        losses = [] # Track losses during adaptation
        max_iters = 20 # Safety measure on iterations to prevent infinite loops
        ...
        while True:
            ...
            # Forward pass through temporary model
            try:
                output = temp_model(sub_data) # Attempt standard PyG Data object input
            except Exception:
                try:
                    # Fallback to common GNN signature (x, edge_index, batch)
                    output = temp_model(sub_data.x, sub_data.edge_index, getattr(sub_data, "batch", None))
                except Exception:
                    # Cannot forward so abort adaptation
                    break
            ...
            try:
                ...
                # if task.loss expects different shapes, try simple cross_entropy
                loss_value = nn.functional.cross_entropy(output, sub_data.y)

            # Backpropagate to compute gradients
            loss_value.backward()

            # Update adaptive weights element-wise (check gradients)
            for pt, p, pg, w in zip(params_tp, params_p, params_gp, self.weights):
                if pt.grad is None: # Skip if gradient is missing
                    continue
                # Ensure same dtype and device
                grad = pt.grad.to(w.dtype).to(w.device)
                # Difference between global and local top layers
                diff = (pg.data - p.data).to(w.dtype).to(w.device)
                # Update weight with small step (eta), clamp between 0 and 1 to prevent instability
                w.data = torch.clamp(w - self.eta * (grad * diff), 0.0, 1.0)

            # Apply updated adaptive weights to temporary model parameters
            for pt, p, pg, w in zip(params_tp, params_p, params_gp, self.weights):
                # Learn a weight w (between 0 and 1) for each parameter in the top layers: 
                # w values are what FedALA learns from the client’s own data
                pt.data = p.data + (pg.data - p.data) * w # Interpolate between local and global layers

            # Record loss for stopping criteria
            losses.append(loss_value.item())

            # Stopping criteria:
            # 1) Not in first round (start_phase = False) or
            # 2) Recent 3 losses are very stable or
            # 3) Reached max iterations
            if (not self.start_phase) or (len(losses) > 3 and np.std(losses[-3:]) < 1e-4) or it >= max_iters:
                break

        # First round adaptation is complete
        self.start_phase = False

        # Apply adapted top layers back to the local model
        for p, pt in zip(params_p, params_tp):
            p.data = pt.data.clone()

        # Update task model with adapted local model
        self.task.model.load_state_dict(local_model.state_dict())

    def send_message(self):
        """Send the locally trained/adapted model back to the server.
        In Federated Learning, the server collects all clients' models for aggregation"""
        print("FedALAClient.send_message() is called.", flush=True)

        # Save the client's model and the number of samples it trained on into the message pool
        self.message_pool[f"client_{self.client_id}"] = {
            # 'num_samples' tells the server how much data this client used for weighting in aggregation
            # If 'num_samples' attribute doesn't exist in the task, default to 1
            "num_samples": getattr(self.task, "num_samples", 1),
            # 'weight' is a full copy of the client's model parameters
            # deepcopy is used so that any future changes on this client don't accidentally modify the saved version
